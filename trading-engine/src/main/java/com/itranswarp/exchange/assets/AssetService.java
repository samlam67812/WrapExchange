...
public class AssetService extends LoggerSupport {
    ...
    // freeze and unfreeze among same person account
    ...
    public void tryTransfer(Transfer type, Long fromUser, Long toUser, AssetEnum assetId, BigDecimal amount) {
    ...
    public boolean tryTransfer(Transfer type, Long fromUser, Long toUser,
                               AssetEnum assetId, BigDecimal amount, boolean checkBalance) {
        if (amount.signum() == 0) {
            return true;
        }
    ...
    private Asset initAssets(Long userId, AssetEnum assetId) {
        ConcurrentHashMap<AssetEnum, Asset> map = userAssets.get(userId);
        if (map == null) {
